...
def build_final_model():
    ...
    print("Generating synthetic target incorporating Sentinel vulnerabilities...")
    risk_condition = (
        (df['slope_max'] > 25) & 
        (df['rainfall_3d'] > df['rainfall_3d'].quantile(0.80)) & 
        ((df['mndwi'] > 0.0) | (df['ndvi'] < 0.2))
    )
    ...
    shap_out = os.path.join(PROCESSED_DIR, "shap_summary_final.png")
